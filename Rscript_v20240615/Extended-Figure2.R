...
# Extended Figure 2. Microbial biogeography and metagenomic provinces (MPs).
# =================================================================================================================================
...
# Read data
phylum2cols <- read.table("data/ex-figure2.umap_stat.cols4phyla.txt", header = TRUE, sep = "\t", comment.char = "") # colors for phyla

ps <- readRDS("data/ex-figure2.umap_stat.braken.RDS") # abundance table

if(file.exists("data/ex-figure2.umap_stat.jsd_dist.RDS")){
  ps.dist <- readRDS("data/ex-figure2.umap_stat.jsd_dist.RDS") %>% as.matrix %>% data.frame
}else{
  ps.dist <- phyloseq::distance(ps, method = "jsd")
  saveRDS(dist, "data/ex-figure2.umap_stat.jsd_dist.RDS")
}

umap.coordinates <- read.table("data/ex-figure2.umap_stat.coords.txt", sep = "\t", header = FALSE, skip = 1, row.names = 1) %>%
  select(V2,V3,V4) %>%
...
pdf("figs/Extended-Figure2.pdf", width = 18, height = 16)
