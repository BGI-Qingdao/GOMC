...
# Extended Figure 3d. The number (the middle heatmap) and fraction of MAGs encoding Cas operon (the upper part) or ARG (the lower part)
...
df.mag <- read.table("data/ex-figure3d.cas_stat.mag_in_umap.txt", header = TRUE, sep = "\t", row.names = 1) %>%
  mutate(umap = factor(row.names(.), levels = row.names(.)), group = 1)

df.arg <- read.table("data/ex-figure3d.cas_stat.arg_in_umap.txt", header = TRUE, sep = "\t", row.names = 1) %>%
  mutate(group = factor(row.names(.), levels = rev(names(env.cols))))

df.cas <- read.table("data/ex-figure3d.cas_stat.cas_in_umap.txt", header = TRUE, sep = "\t", row.names = 1) %>%
  mutate(group = factor(row.names(.), levels = rev(names(env.cols))))
...
pdf("figs/Extended-Figure3d.pdf", width = 14, height = 9, onefile = F)
print(p)
ggdraw() +
