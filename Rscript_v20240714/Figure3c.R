...
# Figure 3c. Barplots display the incidence rate of Cas operon in genomes from different ecosystems.
# ==================================================================================================================================
...
# Read data
df <- read.table("data/figure3c.cas_stat.ecosystem.txt", header = TRUE, sep = "\t", row.names = 1) %>%
...
p <- ggplot() +
  ...
  geom_point(data = df.long %>% filter(group == "Crispr"), aes(x = taxon, y = ratio, color = group), size = 6) +
...
pdf("figs/Figure3c.pdf", width = 12, height = 5, onefile = F)
print(p)
dev.off()
