...
def fetch_bgc_metadata(result_folder):
    with sqlite3.connect(path.join(result_folder, "data.db")) as con:
        cur = con.cursor()        
...
def main():
    try:
        bigslice_result_folder_query = argv[1]
        bigslice_result_folder_sub = argv[2]
        threshold = float(argv[3])
        output_bgc_features_tsv_path = argv[4]
        output_gcf_models_tsv_path = argv[5]
        # output_bgc_membership_path = argv[5]
    except:
        print("usage: python perform_l2norm_clustering.py <bigslice_result_folder_query> <bigslice_result_folder_sub> <threshold_0.0-1.4> <output_bgc_features_tsv_path> <output_gcf_models_tsv_path>")
        return 1

    print("extracting query bgc features...")
    bgc_features_query = fetch_bgc_features(bigslice_result_folder_query)
    bgc_features_query.columns = get_hmm_names(bigslice_result_folder_query)

    print ("saving to file...")
    bgc_features_query.to_csv(output_bgc_features_tsv_path, sep="\t")

    print("extracting query bgc metadata...")
    # bgc_metadata_query = fetch_bgc_metadata(bigslice_result_folder_query).loc[bgc_features_query.index]

    print("extracting sub bgc features...")
    bgc_features_sub = fetch_bgc_features(bigslice_result_folder_sub)
    bgc_features_sub.columns = get_hmm_names(bigslice_result_folder_sub)

    print("extracting sub bgc metadata...")
    bgc_metadata_sub = fetch_bgc_metadata(bigslice_result_folder_sub).loc[bgc_features_sub.index]

    print("calculating GCF features...")
    gcf_models = fetch_gcf_models(
    	normalize(bgc_features_sub[bgc_metadata_sub["contig_edge"] == 0], norm="l2")[
    		np.argsort(bgc_features_sub[bgc_metadata_sub["contig_edge"] == 0].sum(axis=1))
    	], threshold
    )
    gcf_models = pd.DataFrame(gcf_models, columns=bgc_features_sub.columns)

    print ("saving to file...")
    gcf_models.to_csv(output_gcf_models_tsv_path, sep="\t")

    # print("assigning BGC memberships...")
    # bgc_memberships = fetch_gcf_membership(bgc_features_query, gcf_models)
