...
if ($ARGV[2] eq "T"){
	`sed '1d' $ARGV[1]/temp/GCF_counts_per_phylim_total.txt | sort -rnk 2 | awk 'BEGIN{print "Phylum""\t""Number(Total)"}{print}' > $ARGV[1]/temp/GCF_counts_per_phylim_total_sorted.txt `;
}
