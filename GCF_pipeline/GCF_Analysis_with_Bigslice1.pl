...
my ($MAGs,$Antismash,$Phylums,$outdir,$L2norm_threshold,$Endpoint,$Nboot,$SE,$Novetly_threshold,$project,$queue);
...
sub run_bigslice{
    ...
    my $cmd1 = "$bigslice -i $outdir/1_Bigslice_Input $outdir_sub --threshold 9999 --complete -t 16";
    my @out_files = ("$outdir_sub/result/data.db");
    print_cmd($cmd1,$sh_bigslice,\@out_files);
    run_cmd($job,$sh_bigslice,1,16,64,$project,$queue);
}
...
sub run_ed_clustering{
    ...
    run_cmd($job,$sh_clustering,5,16,64,$project,$queue);
}
