...
p2 <- ggplot(data = df.mag.long) +
  ...
  theme(plot.margin = unit(c(0.2, 0.2, 0.2, 0.2), "cm"),
        ...
        axis.ticks.length.y = unit(0.2, "cm"),
        axis.text.x = element_text(size = 12, angle = 90, hjust = 1, vjust = 0.3, colour = NA),
        axis.text.y = element_text(size = 14, color = "#1d4ea1"))

if(!dir.exists("figs")) dir.create("figs")
pdf("figs/Figure3a.pdf", width = 17, height = 8, onefile = F)
ggdraw() +
  draw_plot(ggplotify::as.ggplot(p1), x = 0, y = 0, width = 1, height = 1) +
  draw_plot(ggplotify::as.ggplot(p2), x = 0.046, y = 0, width = 0.9025, height = 1)
dev.off()